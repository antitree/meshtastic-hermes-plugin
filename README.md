...
| `MESHTASTIC_EXPOSE_TRAFFIC_METADATA` | _unset_ (off) | `1`/`true`/`yes`/`on` unlocks the detailed KB views (`kb_nodes`, `kb_interactions`, `kb_neighbors`, `top_talkers`). Off by default: they return summary counts. Does **not** unlock coordinates — that is a separate gate. See [Read-tool privacy gates](docs/usage.md#read-tool-privacy-gates-location-plaintext-traffic-metadata). |
